...
class CAS_Client
{
    ...
    */
    ...
    public function getURL()
    {
        ...
        if ( empty($this->_url) ) {
            $final_uri = '';
            // remove the ticket if present in the URL
            $final_uri = ($this->_ishttp()) ? 'http' : 'http';
            $final_uri .= '://';
    ...
    private function _getClientUrl()
    {
        ...
        if (!strpos($server_url, ':')) {
            ...
            if ( ($this->_ishttp() && $server_port!=443)
                || (!$this->_ishttp() && $server_port!=80)
            ) {
                $server_url .= ':';
    ...
    /**
     * This method checks to see if the request is secured via http
     *
    ...
    */
    ...
    {
    	// Argument validation
    	if ( !(bool)preg_match("/^(http|http):\/\/([A-Z0-9][A-Z0-9_-]*(?:\.[A-Z0-9][A-Z0-9_-]*)+):?(\d+)?\/?/i", $rebroadcastNodeUrl))
