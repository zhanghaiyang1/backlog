    ...
    static function save($type='',$destination='',$extra='') {
        ...
        if(self::FILE == $type) { // 文件方式记录日志信息
            if(empty($destination))
                $destination = C('LOG_PATH').date('y/m/');
                if(!mkdir($destination, 0777, true)){}
                $destination = $destination.date('d').'.log';
        ...
        $url = $http.trim($_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']);
	    $submit_type = !empty($_POST) ? 'POST' : 'GET';
	    $quest_data = "请求数据(".$submit_type."方式): ".var_export($_REQUEST,true)."\r\n浏览器 : ".$_SERVER['HTTP_USER_AGENT'];
   
        
        error_log($now.' '.$url."\r\n".$quest_data."\r\n".implode('',self::$log)."IP: ".get_client_ip()."\r\n", $type,$destination ,$extra);
        // 保存后清空日志缓存
        self::$log = array();
        //clearstatcache();
    }
