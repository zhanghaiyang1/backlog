...
class Db {
        ...
        if ( !empty($db_config) && is_string($db_config)) {
            ...
            $db_config = $this->parseDSN($db_config);
             ...
             $db_config = array(
                  ...
                  'pdo_prepare'    =>  $db_config['pdo_prepare']
             );
        ...
        if(C('DB_RW_SEPARATE')){
            ...
            if($master){
                ...
                if(is_numeric(C('DB_SLAVE_NO'))) {// 指定服务器读
                    ...
                    // 读操作连接从服务器
                    $r = floor(mt_rand(C('DB_MASTER_NUM'),count($_config['dsn'])-1));   // 每次随机连接的数据库
                }
            }
        }else{
            // 读写操作不区分服务器
            $r = floor(mt_rand(0,count($_config['dsn'])-1));   // 每次随机连接的数据库
        }
        $db_config = array(
    ...
    public function select($options=array()) {
        ...
        if($options['is_prepare']){
        	$result   = $this->fquery($sql,$this->parseBind(!empty($options['bind'])?$options['bind']:array()));
        }else{
    ...
    public function buildSelectSql($options=array()) {
        ...
        $db_prefix = strtolower(C('DB_PREFIX'));
        $table_prefix = empty($options['table_prefix']) ? $db_prefix : strtolower($options['table_prefix']);
        $this->model = $options['table'] = empty($options['dataName']) ? $table_prefix.$this->model : $table_prefix.trim($options['dataName']);
		
        if($options['is_sql']){
            $this->selectSql = "SELECT%DISTINCT% %FIELD% FROM ($options[dataName]) t %JOIN%%WHERE%%GROUP%%HAVING%%ORDER%%LIMIT% %UNION%%COMMENT%";
        }
