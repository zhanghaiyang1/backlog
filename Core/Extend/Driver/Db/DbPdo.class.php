...
class DbPdo extends Db{
    ...
    public function __construct($config=''){
        ...
        if($config['pdo_prepare']){
        	$this->is_pdo_prepare = true;
        }
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                ...
                $this->linkID[$linkNum]->setAttribute(PDO :: ATTR_CASE, PDO :: CASE_LOWER);
    ...
    public function query($str,$bind=array()) {
    	if($this->is_pdo_prepare){
	        ...
	        $type = C('DB_DEPLOY_TYPE');
	        if($type && eregi('^(insert|update|delete)(.*)',trim($str))){
	        ...
	        if ( false === $result ) {
	        	...
	        	if(eregi('^(insert|update|delete)(.*)',trim($str))){
	        		return true;
	        	}
	        	return $this->getAll();
	        }
     ...
     * @return mixed
     */
    public function fquery($str,$binds=array(),$one=false) {
	 	$type = C('DB_DEPLOY_TYPE');
        if($type && eregi('^(insert|update|delete)(.*)',trim($str))){
        	$this->initConnect(true);
        }else{
        ...
        if(false === $this->PDOStatement)
            throw_exception($this->error());
        $result =   $this->PDOStatement->execute($binds);
        ...
        if ( false === $result ) {
        	...
        	if(eregi('^(insert)(.*)',trim($str))){
        		 $this->lastInsID = $this->getLastInsertId(); 
        		 return empty($this->lastInsID) ? true : $this->lastInsID;
        	}else if(eregi('^(update|delete)(.*)',trim($str))){
        		 return $result;
        	}else{
    ...
    public function execute($str,$bind=array()) {
        ...
        if ( false === $result) {
            ...
            if($flag || preg_match("/^\s*(INSERT\s+INTO|REPLACE\s+INTO)\s+/i", $str)) {
                return $this->lastInsID = $this->getLastInsertId();
            }
            return $result;//$this->numRows;
        }
    }
     ...
     * @return integer
     */
    public function getOLastInsertId($sequenceName) {
    	 if(empty($sequenceName)) return false;
         switch($this->dbType) {
            case 'PGSQL':
            case 'SQLITE':
            case 'MSSQL':
